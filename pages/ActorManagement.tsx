...
const ActorListItem = memo(({ actor, onClick, borderColor, isLast }: { actor: any, onClick: () => void, borderColor: string, isLast: boolean }) => {
...
const ActorCardItem = memo(({ actor, onClick, borderColor }: { actor: any, onClick: () => void, borderColor: string }) => {
  const imgSrc = actor.originalImageUrl || actor.imageUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(actor.name)}&background=random`;
  const [isLoaded, setIsLoaded] = useState(false);
...
export const ActorManagement: React.FC = React.memo(() => {
  ...
  const filteredActors = useMemo(() => {
    let actors = state.actors.filter(a => !a.isDeleted);
  ...
  }, [state.actors, deferredSearchQuery, sortOrder]);
  ...
  const visibleActors = useMemo(() => filteredActors.slice(0, visibleCount), [filteredActors, visibleCount]);
  ...
  }, [visibleActors.length, isReady]);

  const handleActorClick = useCallback((id: string) => {
    ...
    >
      {managementView === 'List' ? (
        <div className="flex flex-col relative min-h-[500px]">
          ...
          {isReady && visibleCount < filteredActors.length && (
      ...
      {isReady && filteredActors.length === 0 && (
