...
export const StudioLinks: React.FC = React.memo(() => {
  const { id } = useParams();
  ...
  const associatedLinks = useMemo(() => {
    ...
    return state.links.filter(l => !l.isDeleted && l.studioIds.includes(id));
  ...
  const studio = useMemo(() => studioMap.get(id || ''), [studioMap, id]);
  ...
  if (!studio) return <Navigate to="/manage-studios" replace />;
